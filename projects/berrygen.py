...
class BerryProject:
    ...
    def sync_project(self):
        ...
        self.project_init = importlib.import_module('{}.{}'.format(self.project_name, '__init__'))
    ...
    def _update_project_files_(self):
        ...
        with open(join(self.path, self.INIT_FILE_NAME), 'w') as file:
            file.write(init_text)

    def _make_berry_file(self, berry_type, nameset):
    ...
    def _gen_files_(self):
        ...
        shutil.copyfile(join(self.GEN_SRC_PATH, self.INIT_FILE_NAME),
                        join(self.path, self.INIT_FILE_NAME))
        shutil.copyfile(join(self.GEN_SRC_PATH, self.MAIN_FILE_NAME),
                        join(self.path, self.MAIN_FILE_NAME))

    def _check_files_(self):
        if not os.path.exists(self.path):
            self._gen_files_()
            return
        if not (os.path.exists(join(self.path, self.INIT_FILE_NAME)) and os.path.exists(
                join(self.path, self.MAIN_FILE_NAME))):
